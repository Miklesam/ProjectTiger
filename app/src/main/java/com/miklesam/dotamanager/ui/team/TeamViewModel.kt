...
class TeamViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private var teamRepository: TeamsRepository = TeamsRepository(application)

    fun getPlayer(): LiveData<List<Player>> {
        return repository.getPlayers()
    }

    fun getPlayersByNickNames(): LiveData<List<Player>> {
    ...
    fun deletePlayerByNickName(player: String) {
        repository.deletePlayerByNickName(player)
    }
